...
// Placeholder requires for future multi-backend support
...
class LLMService {
  constructor() {
    ...
    this.requestCount = 0;
    this.errorCount = 0;
    this.backend = 'gemini'; // 'gemini' | 'openai' | 'anthropic' | 'local'
    
    this.initializeClient();
  }

  initializeClient() {
    // Select backend from config, defaulting to gemini for backwards compatibility
    const backendFromConfig = (config.get('llm.backend') || 'gemini').toString().toLowerCase();
    const supportedBackends = ['gemini', 'openai', 'anthropic', 'local'];
    this.backend = supportedBackends.includes(backendFromConfig) ? backendFromConfig : 'gemini';

    logger.info('Initializing LLM backend', { backend: this.backend });

    if (this.backend === 'gemini') {
      this.initializeGeminiClient();
    } else {
      // Skeleton for future backends: keep service non-breaking but mark as uninitialized
      logger.warn('Selected backend is not fully implemented yet; falling back to Gemini', {
  ...
  async processImageWithSkill(imageBuffer, mimeType, activeSkill, sessionMemory = [], programmingLanguage = null) {
    if (!this.isInitialized) {
      throw new Error('LLM service not initialized. Check API key/backend configuration.');
    }
...
Remember: Be intelligent about filtering - only provide detailed responses when the user actually needs help with ${activeSkill}.`;
  ...
  updateApiKey(newApiKey) {
    // For now, only Gemini is wired; future backends can extend this method or add new ones
    process.env.GEMINI_API_KEY = newApiKey;
    this.isInitialized = false;
    this.initializeClient();
    
    logger.info('API key updated and client reinitialized', { backend: this.backend });
  }
  ...
  getStats() {
    return {
      ...
      config: {
        backend: config.get('llm.backend') || 'gemini',
        gemini: config.get('llm.gemini')
      }
    };
  }
