  ...
  async checkScreenSharingStatus() {
    try {
      const sources = await desktopCapturer.getSources({
        // We only need screen sources here; window list can be large and noisy
        types: ['screen'],
      ...
      // Heuristic: if we can successfully enumerate one or more screen sources,
      // assume a system-level screen capture/sharing session is active.
      // On many platforms, when no sharing is active, this list may be empty
      // or require an explicit user gesture.
      const isNowSharing = Array.isArray(sources) && sources.length > 0;

      if (isNowSharing !== wasSharing) {
        this.isScreenBeingShared = isNowSharing;
        ...
        logger.info('Screen sharing state changed', {
          wasSharing,
          isNowSharing,
